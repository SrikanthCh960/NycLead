...
import { Mail, MapPin, Phone, ArrowRight, Clock, Calendar, Sparkles } from "lucide-react";
...
function ContactInfoCard() {
  ...
  const contactItems = [
    ...
    { icon: MapPin, label: "Office",   value: CONTACT_LOCATION,   href: null,                      clickable: false },
    { icon: Phone,  label: "Phone",    value: "+1 (212) 555-0100", href: "tel:+12125550100",        clickable: true },
  ];
...
function QuickCTACard() {
  ...
  return (
    <motion.div
      ...
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => { onMouseLeave(); setHovered(false); }}
      className="relative rounded-2xl p-7 overflow-hidden cursor-default will-change-transform"
      style={{
        rotateX, rotateY, transformStyle: "preserve-3d",
        background: "#ffffff",
        border: `1px solid ${hovered ? "rgba(37,99,235,0.22)" : "rgba(0,0,0,0.06)"}`,
        boxShadow: hovered ? "0 12px 40px rgba(37,99,235,0.09)" : "0 2px 12px rgba(0,0,0,0.04)",
        transition: "border-color 0.3s, box-shadow 0.3s",
      }}
      whileHover={{ y: -6, scale: 1.01 }}
      transition={{ type: "spring", stiffness: 200, damping: 22 }}
    >
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <span className="text-slate-800 font-semibold text-sm">Book a Strategy Call</span>
      </div>
      <p className="text-slate-500 text-xs leading-relaxed mb-4">
        30-minute intro call with a senior consultant. No commitment required.
      </p>
      <a
        href={`mailto:${CONTACT_EMAIL}?subject=Strategy%20Call%20Request`}
        className="group inline-flex items-center gap-1.5 text-blue-600 text-xs font-semibold hover:gap-2.5 transition-all duration-300"
      >
        Schedule now
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div ref={headerRef} className="text-center mb-20">
          ...
          >
            ...
            <span className="text-blue-700 text-xs font-semibold tracking-[0.18em] uppercase">Get In Touch</span>
          </motion.div>
          ...
          <div className="overflow-hidden mb-5">
            <motion.h1
              ...
              className="text-4xl md:text-5xl lg:text-[3.4rem] font-bold text-slate-900 leading-[1.06] tracking-[-0.025em]"
            >
          ...
          <motion.p
            ...
            className="text-slate-500 text-[1.05rem] max-w-xl mx-auto leading-[1.85]"
          >
